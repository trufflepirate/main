...
By: CS2113-AY1819S1-T09-1      Since: `Aug 2018`      Licence: `MIT`
...
* exposes unmodifiable `ObservableList<Person>`, `ObservableList<Admin>`, `ObservableList<Machine>`, `ObservableList<Job>` which can be 'observed' e.g. the UI can be bound to these lists so that the UI automatically updates when the data in the lists change.
...
* can segregate different data into separate files
...
// tag::addMachines[]
=== Add Machines feature
==== Current Implementation

Adding machines is facilitated by `addMachineCommand`. It extends command with the instantiation of a new `machine`, and addition of the new `machine` to the model. Additionally, the class contains all the static information that describes the command word and syntax, proper use of the command, and syntax error messages.

The main feature of the `addMachineCommand` class is `execute(Model, CommandHistory)`.

The following describes a main success case of the `addMachineCommand`, from the user input to command execution, assuming that the app has already initialized:

Step 1. User keys in addMachine command and presses enter. The `commandBox` object will call `execute()` of the `LogicManager` object, passing the user input as a parameter.

Step 2. `LogicManager.execute()` calls the `parseCommand()` method of `AddressBookParser` object, passing the user input as a parameter.

Step 3. `parseCommand()` will then compare the command word of the user input using regex, and instantiate the appropriate `commandParser`. In this case, the `addMachineCommandParser` will be instantiated. `parseCommand()` will then return `addMachineCommandParser.parse()`, with the user input being passed in as parameters of `parse()`.

Step 4. `Parse()` will split the user input into is component tokens using `ArgumentTokenizer()`. It will then create new instances of `MachineName` object, `MachineStatus` object, an `Arraylist` of `Jobs`, a `Set` of `Tags`. The pointers to these objects will be passed into the constructor for a new `Machine` object. Finally, `Parse()` will then instantiate and return a new `AddMachineCommand` object, passing the `Machine` object into its constructor.

Step 5.  This `addMachineCommand` is the returned to `LogicManager.execute()`, with in turn calls and returns `addMachineCommand.execute()`, passing the pointer to the `Model`, and the new `Machine` object as parameters.

Step 6. `addMachineCommand.execute()` adds the new `Machine` object into the model via `model.addMachine()`. It then returns a new `CommandResult` containing pointers to the Success Message and the Machine object.
